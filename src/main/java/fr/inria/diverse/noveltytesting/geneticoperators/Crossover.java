    ...
    public void process(Population population) {
        ...
        if (interfaces.size() > 1) {
            for (int i = 0; i < interfaces.size() - 1; i++) {
                ...
                int crossoverMethodPosition = (int) (Math.random() * interfaces.get(i).getMethods().size());
                ...
                interfaces.get(i).getMethods().get(crossoverMethodPosition).getParameters().get(crossoverPoint).setValue(
